...
# lat/lon for deployment 1 is wrong in file, used 2052 sounding location
	lat = 34.5303
	lon = -99.5088
...
# # 
	...
	if time == '2155':
		hour = 2130
		t = n + 5
		lat = lidarfile.variables['lat'][30]
		lon = lidarfile.variables['lon'][30]
	if time == '2200':
		hour = 2130
		t = n + 6
		lat = lidarfile.variables['lat'][30]
	...
	outdir = '{}/Pat_20th_rerun/Lidar WoFS Sounding Files/'.format(pat_path)
	...
	stormpath = '{}/Pat_20th_rerun/Real Storm Locations/RealStormLatLon_{}.csv'.format(pat_path,date)
	...
	LocFile = '{}/Pat_20th_rerun/Storm Objects Locations/{}_{}.csv'.format(pat_path,hour,time)
	...
	for f, infile in enumerate(files):
		xdex = Gridx[f]
		ydex = Gridy[f]
		print('lat/lon indicies for member: ' + str(Memlist[f+1]) + ": " + str(xdex) + ',' + str(ydex))
	...
	##################### Get dimensions and attributes using first wrfwof file ########################
		  ...
		  if (len(timestep) == 1):
			 timestep = '0' + timestep
		  outname = "wofs_SND_" + timestep + "_" + init_date + "_" + init_time + "_" + valid_time + "_" + time + "_Relative_Lidar.nc"         #output file
